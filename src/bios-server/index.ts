...
import { exportResource } from "../utils";
import { DEFAULT_ERROR } from "../constants/resData";
...
export default class BiosServer extends Koa {
  ...
  constructor(option: TOption) {
    super();
    const { paths, config, resources } = option;
    this.paths = paths || {};
    this.config = config;
  ...
  private parser() {
    ...
    serviceResources = Object.keys(serviceResources).reduce(
      (prev, parentKey) => {
    ...
    this.resources.middleware = Object.assign(
      this.resources.middleware || {},
      middleResources
    );
    this.resources.router = Object.assign(
      this.resources.router || {},
      routerResources
    );
    this.resources.service = Object.assign(
  ...
  private register() {
    ...
    routes.forEach(route => {
      ...
      router[method](`/${name}`, ...mids, async (ctx: TContext) => {
        try {
          return await handler.call(this, route, ctx);
        } catch (error) {
          console.error(error);
          return (ctx.body = DEFAULT_ERROR);
        }
      });
    });
